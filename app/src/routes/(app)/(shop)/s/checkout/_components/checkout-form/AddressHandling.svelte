...
{#if !cart?.shipping_address || is_editing}
...
{:else}
	<div class="border-accent relative h-fit rounded-2xl border px-4 py-3">
		<div class="absolute bottom-0 right-3">
			<button class="text-accent-400 text-xs font-light" on:click={() => (is_editing = true)}>
				Bearbeiten
			</button>
		</div>
		<p>{cart.shipping_address.first_name} {cart.shipping_address.last_name}</p>
		<p>{cart.shipping_address.address_1}</p>
		<p>{cart.shipping_address.city}, {cart.shipping_address.postal_code}</p>
