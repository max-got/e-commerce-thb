<script lang="ts">
	import { navigating, page } from '$app/stores';
	import { loading } from '$lib/stores/loading';
	import LogOutLink from '../auth/LogOutLink.svelte';
</script>

<div class="bg-accent relative">
	<div class="loadingbar_indicator" data-active={$navigating || $loading}></div>

	<div class="max-w-layout relative z-10 mx-auto flex items-center justify-end gap-4 px-4 py-1">
		<div class="flex items-center gap-2 sm:flex">
			<a
				class="hover:bg-accent-100 hover:text-accent after:bg-accent-300 relative hidden rounded-sm px-2 py-px text-xs text-white no-underline after:absolute after:-right-1 after:bottom-1 after:top-1 after:block after:w-px last-of-type:after:content-none md:block"
				href="/s/search"
			>
				Suche
			</a>
			<a
				class="hover:bg-accent-100 hover:text-accent after:bg-accent-300 relative hidden rounded-sm px-2 py-px text-xs text-white no-underline after:absolute after:-right-1 after:bottom-1 after:top-1 after:block after:w-px last-of-type:after:content-none md:block"
				href="/#faq"
			>
				Fragen und Antworten
			</a>
			{#if !$page.data.user}
				<a
					class="hover:bg-accent-100 hover:text-accent after:bg-accent-300 relative rounded-sm px-2 py-px text-xs text-white no-underline after:absolute after:-right-1 after:bottom-1 after:top-1 after:block after:w-px last-of-type:after:content-none"
					href="/auth/register"
				>
					Registrieren
				</a>
				<a
					class="hover:bg-accent-100 hover:text-accent after:bg-accent-300 relative rounded-sm px-2 py-px text-xs text-white no-underline after:absolute after:-right-1 after:bottom-1 after:top-1 after:block after:w-px last-of-type:after:content-none"
					href="/auth/login"
				>
					Anmelden
				</a>
			{:else}
				<LogOutLink
					colorway="simple"
					class="hover:bg-accent-100 hover:text-accent after:bg-accent-300 relative rounded-sm px-2 py-px text-xs text-white no-underline after:absolute after:-right-1 after:bottom-1 after:top-1 after:block after:w-px last-of-type:after:content-none"
				/>
			{/if}
		</div>
	</div>
</div>

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 239.8 95.2" style="display:none;">
	<defs>
		<g id="zenit_logo_defs" fill="currentColor">
			<path
				d="M90 26c-18 0-27 10-32 32l-2 19H32l31-51 4-15H19l-4 18h22L3 82 0 95h80c21 0 25-13 28-22H85c-2 5-3 7-5 7-1 0-2-1-2-4l2-10h29l3-11c4-18-3-29-22-29Zm-1 27h-6c2-10 3-12 5-12s2 1 2 3l-1 9Z"
			/>
			<path
				d="m114 49 5-22h18c3 0 5 3 4 5 3-3 8-6 15-6 11 0 15 9 13 21l-11 47h-23l9-45c1-4 1-6-2-6-2 0-3 2-4 7l-9 44h-24l9-45Z"
			/>
			<path d="m198 27-4 2-2 1a11 11 0 0 1-11-1l-4-2h-1l-15 67h24l14-67h-1Z" />
			<path
				d="M202 27h7l3-14h23l-3 14h8l-4 18h-8l-6 28c0 4 1 5 4 5l3-1-3 17-13 1c-14 0-18-5-15-18l7-32h-7l4-18Z"
			/>
			<path d="m195 24-3 2a11 11 0 0 1-9 0l-3-2c-3-3-4-8-1-12l8-12 9 12c3 4 2 9-1 12Z" />
		</g>
	</defs>
</svg>

<style lang="postcss">
	.loadingbar_indicator {
		animation: loadingbar 1.5s linear infinite alternate-reverse;
	}

	@keyframes loadingbar {
		0% {
			background-size: 200% 200%;
		}
		100% {
			background-size: 100% 100%;
		}
	}

	.loadingbar_indicator[data-active='false'] {
		opacity: 0;
		animation: none;
		pointer-events: none;
	}

	.loadingbar_indicator {
		--color: theme(colors.black);
		--indicator-color: theme(colors.primary.100);
		--height: 0.25rem;
		position: fixed;
		transition: all 0.2s ease-in-out;
		z-index: 100;
		bottom: calc(var(--height) * -1);
		top: 0;
		height: var(--height);
		left: 0;
		right: 0;
		display: block;
		overflow: hidden;
		background-color: var(--color);
	}

	.loadingbar_indicator::after {
		content: '';
		position: absolute;
		inset: 0;
		transform: translateX(-100%);
		background-image: linear-gradient(
			90deg,
			color-mix(in srgb, var(--indicator-color), var(--color) 0%) 0,
			color-mix(in srgb, var(--indicator-color), var(--color) 10%) 50%,
			color-mix(in srgb, var(--indicator-color), var(--color) 20%) 100%
		);
		animation: shimmer 3s ease-out infinite;
	}

	@keyframes shimmer {
		100% {
			transform: translateX(0%);
			opacity: 0;
		}
	}
</style>
